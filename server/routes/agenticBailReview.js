/*
 * Agentic bail review workflow: Multi-agent risk, compliance, and judge-recommendation coordination
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Multi-agent risk, compliance, and judge-recommendation coordination
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'agentic_bail_review' });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${'Agentic bail review workflow'}\n\nReturn JSON {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const systemPrompt = `You are a $'senior bail-bond reviewer running a multi-agent pipeline over defendant facts'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'agentic_bail_review', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'agentic_bail_review', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
