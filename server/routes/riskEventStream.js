/*
 * Real-time risk-event stream: Ingest new arrest, travel, address change events and trigger re-assessment
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Ingest new arrest, travel, address change events and trigger re-assessment
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'risk_event_stream' });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${'Real-time risk-event stream'}\n\nReturn JSON {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const systemPrompt = `You are a $'risk analyst evaluating defendant re-assessment triggers'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'risk_event_stream', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'risk_event_stream', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
