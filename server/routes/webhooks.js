...
async function ensureWebhooksTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS webhooks (
      ...
      user_id INTEGER,
      url TEXT NOT NULL,
      events TEXT[] NOT NULL DEFAULT ARRAY['risk_assessment.created']::text[],
      secret VARCHAR(255),
...
const ALLOWED_EVENTS = [
  'risk_assessment.created', 'risk_assessment.updated',
  'bail_bond.created', 'bail_bond.forfeited',
  'flight_risk.alert', 'court_case.hearing_scheduled',
...
router.post('/', async (req, res) => {
  try {
    ...
    const incoming = Array.isArray(events) && events.length > 0 ? events : ['risk_assessment.created'];
...
// POST /:id/test - synth payload (no real HTTP call)
router.post('/:id/test', async (req, res) => {
  try {
    ...
    const payload = {
      ...
      data: { message: 'Test webhook from AIBailBondRiskAssessor' }
    };
...
// GET /events - list available webhook events
