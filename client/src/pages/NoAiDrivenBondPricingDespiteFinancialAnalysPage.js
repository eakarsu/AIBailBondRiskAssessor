...
export default function NoAiDrivenBondPricingDespiteFinancialAnalysPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-ai-driven-bond-pricing-despite-financial-analys/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No AI-driven bond pricing despite financial-analysis data</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-ai-driven-bond-pricing-despite-financial-analys/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
