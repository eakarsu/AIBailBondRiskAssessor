...
// Feature: Historical case RAG over precedents and comparable bail decisions
import React, { useState } from 'react';
import api from '../services/api';

export default function HistoricalCaseRagOverPrecedentsAndComparablePage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-historical-case-rag-over-precedents-and-comparable/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Historical case RAG over precedents and comparable bail decisions</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-historical-case-rag-over-precedents-and-comparable/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
