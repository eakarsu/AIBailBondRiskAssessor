...
// Feature: Agentic bail review workflow with multi-agent risk, compliance, and judge-recommendation coordination
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticBailReviewWorkflowWithMultiAgentRiskPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-bail-review-workflow-with-multi-agent-risk/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic bail review workflow with multi-agent risk, compliance, and judge-recommendation coordination</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-bail-review-workflow-with-multi-agent-risk/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
