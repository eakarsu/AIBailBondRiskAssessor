...
// Feature: Real-time risk-event streaming (new arrests, travel, address changes) triggering re-assessment
import React, { useState } from 'react';
import api from '../services/api';

export default function RealTimeRiskEventStreamingNewArrestsTravelPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-real-time-risk-event-streaming-new-arrests-travel-/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time risk-event streaming (new arrests, travel, address changes) triggering re-assessment</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-risk-event-streaming-new-arrests-travel-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
